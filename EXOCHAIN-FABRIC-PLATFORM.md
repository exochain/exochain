...
- Forensic Evidence: Governed interactions produce evidence through their implemented audit and bundle paths. Cryptographic integrity and replay checks do not establish legal admissibility; the implemented representation and archive-design limits are stated in §19.
...
| ✦ CGRProof † ✦ | Design term for evidence of constitutional invariant preservation. In 0.2.6, the implemented `CgrReductionTrace` proves supplied reduction consistency by replay; it is not a kernel-signed proof of invariant preservation. See §19.6 for the exact boundary. |
...
- Holon steps retain reduction traces and event payloads as described in §19.6; transport signing and durable event persistence are separate runtime-path responsibilities

3A.2.3 Judicial Branch: CGR Kernel

- IMMUTABLE mathematical verifier—the 'Supreme Court' of the system
- Performs runtime invariant checks; the implemented reduction trace records their results as hash-bound metadata, not as a kernel-signed CGRProof (see §19.6)
...
The following is a design flow. Its `CGRProof` labels do not establish a
kernel-signed proof producer or consensus validation of such a proof in 0.2.6.
The implemented runtime-check and reduction-evidence boundary is specified in
§19.6.
...
| ✦ Invariant Bypass ✦ | Low | Catastrophic | Runtime kernel adjudication and invariant checks. The §19.6 reduction trace does not authenticate those checks; kernel signatures, validator verification of such signatures, and formal correctness proof are not established by the 0.2.6 trace implementation. |
...
Evidence bundles carry ledger data and verification material for technical
review. Legal admissibility depends on the applicable process and is not
established by this format. The ZIP layout below is a design reference, not an
archive exporter shipped in 0.2.6; the implemented `exo-legal` bundle supports
in-memory assembly, hashing, signature verification, and rendering. See §19.6
for the implemented CGR trace and replay contract.
...
The checks below describe technical evidence review. Passing them does not
establish court admissibility or Article 26 certification. In particular, the
implemented reduction trace has no kernel signing key or kernel signature.

| # | Criterion | Verification Method |
|---|-----------|---------------------|
| 1 | Authenticity | Verify supplied bundle signatures against independently trusted signer keys; a trace hash alone does not authenticate a kernel |
...
| 5 | Reproducibility | Supplied combinator and input reproduce the sealed reduction trace and output hash |
| 6 | Timeliness | HLC timestamps within acceptable bounds |

19.6 Extended Evidence Bundles for Holon Actions

Implemented contract in 0.2.6: `reduce_with_trace` produces a hash-sealed
`CgrReductionTrace`. Holon execution retains that trace with recorded invariant
results and the kernel constitution hash. `EvidenceBundle.cgr_program` carries
`CgrProgramEvidence { combinator, input, trace }`; bundle assembly and verification
replay the program, and the bundle hash covers that evidence. These capabilities
were introduced in 0.2.4.

Replay establishes consistency of the supplied program, input, reduction steps,
and output. It does not independently authenticate a kernel signer or rerun
constitutional adjudication; recorded invariant results and the constitution
hash are hash-bound metadata. The Holon producer performs adjudication before
reduction and records its subsequent invariant evaluation.

The archive layout below is a design reference. Version 0.2.6 does not generate
this signed ZIP layout, `kernel_attestation.sig`, bundled `verify.sh`, or
`kernel_binary.wasm`. Consumers must use the implemented bundle representation
and replay API rather than require these nonexistent archive files.
...
The reference archive design specifies the following verification files; the
current implementation does not generate this directory:
...
This repeats the technical review criteria and carries the implementation limits
in §19.5 and §19.6. It does not certify legal admissibility.
