...
pub fn quantize_to_histogram(values: &[u64], buckets: usize) -> Vec<u32> {
    ...
    for &v in values {
        ...
        let idx = ((v - min) as u128 * buckets as u128 / (range as u128 + 1)) as usize;
...
pub fn compute_baseline_similarity(
    ...
    let matching: Vec<&BehavioralSample> = prior_samples.iter()
    ...
    let sum: u64 = matching.iter().map(|s| {
        byte_similarity(new_hash.as_bytes(), s.sample_hash.as_bytes()) as u64
    }).sum();

    Some((sum / matching.len() as u64) as u32)
}
...
fn byte_similarity(a: &[u8; 32], b: &[u8; 32]) -> u32 {
    let sum_min: u32 = a.iter().zip(b.iter()).map(|(&ai, &bi)| ai.min(bi) as u32).sum();
    let sum_max: u32 = a.iter().zip(b.iter()).map(|(&ai, &bi)| ai.max(bi) as u32).sum();
    if sum_max == 0 {
        return 10_000;
    }
    (sum_min as u64 * 10_000 / sum_max as u64) as u32
}
