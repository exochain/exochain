...
export const exampleResponsesConfig = (
  fetchImpl: FetchLike,
...
export async function runOpenAIResponsesExample(
  fetchImpl: FetchLike,
  authority: ReceiptAuthority,
): Promise<unknown> {
  const client = createReceiptedOpenAIClient(exampleResponsesConfig(fetchImpl, authority), {
