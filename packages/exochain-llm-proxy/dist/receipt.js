...
import { LYNK_RECEIPT_RESPONSE_ATTESTATION_DOMAIN, LYNK_RECEIPT_RESPONSE_ATTESTATION_SCHEMA_VERSION, assertJsonUnicodeScalars, verifyLynkReceiptResponseAttestation, } from "./attestation.js";
import { encodeReceiptIntentForWire, } from "./wire.js";
...
export async function emitUsageReceipt(config, receiptIntent) {
    ...
    try {
        ...
        attestedRequest = JSON.parse(serialized);
        ...
        throw new ReceiptEmissionError("EXOCHAIN LYNK receipt intent was malformed or not wire-compatible", receiptIntent.llm_usage_evidence.evidence.idempotency_key_hash, receiptIntent);
    }
    ...
    try {
        ...
        return await validateCommittedReceiptResponse(payload, receiptIntent, config, attestedRequest);
    }
    catch {
        throw new ReceiptEmissionError("EXOCHAIN LYNK receipt response was malformed or untrusted", receiptIntent.llm_usage_evidence.evidence.idempotency_key_hash, receiptIntent, response.status);
    }
}
export function requireProductionValidatorTrust(config) {
    if (config.mode !== "production") {
        return;
    }
    if (typeof config.trustedValidatorDid !== "string"
        || !/^did:[a-z0-9]+:[^\s]+$/.test(config.trustedValidatorDid)) {
        throw new LynkConfigurationError("production LYNK receipt release requires trustedValidatorDid");
    }
    if (typeof config.trustedValidatorPublicKey !== "string"
        || !/^[0-9a-f]{64}$/.test(config.trustedValidatorPublicKey)
        || /^0+$/.test(config.trustedValidatorPublicKey)) {
        throw new LynkConfigurationError("production LYNK receipt release requires a canonical nonzero trustedValidatorPublicKey");
    }
}
export async function resolveReceiptPending(config, pending) {
    return emitUsageReceipt(config, pending.receiptIntent);
}
async function validateCommittedReceiptResponse(payload, receiptIntent, config, wireRequest) {
    ...
    if (!Number.isSafeInteger(response.exochain_finality_height)
        || response.exochain_finality_height <= 0) {
    ...
    if (requireHashBytes(validation.credential_id) !== credentialHash
    ...
    if (attestation.domain !== LYNK_RECEIPT_RESPONSE_ATTESTATION_DOMAIN
    ...
    await verifyLynkReceiptResponseAttestation(config, {
        receiptHash,
        finalityHash,
    ...
    return {
        ...
        exochain_finality_height: response.exochain_finality_height,
...
function requireTimestamp(value) {
    ...
    if (!Number.isSafeInteger(timestamp.physical_ms)
        || timestamp.physical_ms < 0
        || !Number.isSafeInteger(timestamp.logical)
        || timestamp.logical < 0) {
        throw new Error("expected a non-negative HLC timestamp");
    }
}
function requireTimestampProvenance(receipt) {
    ...
    requireExactKeys(proof, ["authority_did", "subject_hash", "issued_at", "signature"], isRfc3161 ? ["proof_kind", "rfc3161"] : []);
    ...
    if (receiptTimestamp.physical_ms !== proofTimestamp.physical_ms
...
function requireRfc3161Proof(value) {
    ...
    if (!isCanonicalNonzeroHash(proof.message_imprint_sha256_hex)
    ...
    if (proof.tsa_trust_anchor_kind !== undefined
...
function requireByteArray(value, minimumLength, maximumLength, rejectAllZero) {
    if (!Array.isArray(value)
