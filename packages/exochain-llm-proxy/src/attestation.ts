...
export const LYNK_RECEIPT_RESPONSE_ATTESTATION_DOMAIN =
...
export function assertJsonUnicodeScalars(value: unknown): void {
 ...
 */
export function encodeLynkReceiptResponseAttestationPayload(
  input: LynkReceiptResponseAttestationInput,
): Uint8Array {
...
function jsonTransportCopy(value: unknown): unknown {
  ...
  if (typeof value === "object") {
    ...
    const copied = Object.create(null) as JsonRecord;
...
export async function verifyLynkReceiptResponseAttestation(
  ...
  if (
    typeof trustedPublicKey !== "string"
    || !PUBLIC_KEY_HEX.test(trustedPublicKey)
    || /^0+$/.test(trustedPublicKey)
  ) {
    throw new Error(
      "LYNK receipt verification requires a canonical nonzero trustedValidatorPublicKey",
    );
  }
  if (
    signature.length !== 64
  ...
  try {
    verified = await subtle.verify(
      ED25519,
      publicKey,
      Uint8Array.from(signature) as BufferSource,
...
function appendCanonicalCbor(value: unknown, output: number[]): void {
  if (value === null) {
    output.push(0xf6);
    return;
  }
...
function appendTypeAndLength(majorType: number, value: number, output: number[]): void {
  ...
  if (value <= 0xffff_ffff) {
    output.push(
      prefix | 26,
      Math.floor(value / 0x1_000_000) & 0xff,
