...
export type WireSignature =
  | { Ed25519: number[] }
  | { PostQuantum: number[] }
  | { Hybrid: { classical: number[]; pq: number[] } };

export interface WireEncryptedPayloadRef
  extends Omit<
    EncryptedPayloadRef,
    "ref_id_hash" | "ciphertext_hash" | "storage_policy_hash" | "key_policy_hash"
  > {
...
export interface WireLlmUsageEvidence
  extends Omit<
    LlmUsageEvidence,
...
export interface WireLlmUsageEvidenceEnvelope
  extends Omit<LlmUsageEvidenceEnvelope, "evidence"> {
 ...
 */
export function encodeReceiptIntentForWire(intent: ReceiptIntent): ReceiptEmitWireRequest {
  return {
    ...
    ...(intent.subject_public_key === undefined
      ? {}
      : {
          subject_public_key: decodeFixedHex(
            intent.subject_public_key,
            32,
            "subject_public_key",
            true,
          ),
    ...
    ...(intent.adapter_public_key === undefined
      ? {}
      : {
          adapter_public_key: decodeFixedHex(
            intent.adapter_public_key,
            32,
            "adapter_public_key",
            true,
          ),
...
function encodeEvidenceEnvelope(
  envelope: LlmUsageEvidenceEnvelope,
): WireLlmUsageEvidenceEnvelope {
  ...
  return {
    ...
    evidence: {
      ...
      ...encodeOptionalHash(
        "provider_request_id_hash",
        evidence.provider_request_id_hash,
      ),
      ...encodeOptionalHash("session_id_hash", evidence.session_id_hash),
      idempotency_key_hash: encodeHash(
        evidence.idempotency_key_hash,
        "llm_usage_evidence.evidence.idempotency_key_hash",
      ),
      action_id: encodeHash(
        evidence.action_id,
        "llm_usage_evidence.evidence.action_id",
      ),
      prompt_hash: encodeHash(
        evidence.prompt_hash,
        "llm_usage_evidence.evidence.prompt_hash",
      ),
      ...
      ...(evidence.encrypted_payload_refs.length === 0
        ...
        : {
            encrypted_payload_refs: evidence.encrypted_payload_refs.map((reference, index) => ({
              ref_id_hash: encodeHash(
                reference.ref_id_hash,
                `llm_usage_evidence.evidence.encrypted_payload_refs[${index}].ref_id_hash`,
              ),
              ciphertext_hash: encodeHash(
                reference.ciphertext_hash,
                `llm_usage_evidence.evidence.encrypted_payload_refs[${index}].ciphertext_hash`,
              ),
              storage_policy_hash: encodeHash(
                reference.storage_policy_hash,
                `llm_usage_evidence.evidence.encrypted_payload_refs[${index}].storage_policy_hash`,
              ),
              key_policy_hash: encodeHash(
                reference.key_policy_hash,
                `llm_usage_evidence.evidence.encrypted_payload_refs[${index}].key_policy_hash`,
              ),
              payload_kind: reference.payload_kind,
              byte_length: reference.byte_length,
            })),
          }),
      custody_policy_hash: encodeHash(
        evidence.custody_policy_hash,
        "llm_usage_evidence.evidence.custody_policy_hash",
      ),
...
): Partial<Record<typeof name, WireHash256>> {
  if (value === undefined) {
    return {};
  }
  return {
    [name]: encodeHash(value, `llm_usage_evidence.evidence.${name}`),
  } as Partial<Record<typeof name, WireHash256>>;
}

function encodeValidationRequest(value: unknown): unknown {
  if (!isRecord(value)) {
    return value;
  }
  const encoded: JsonRecord = { ...value };
...
function encodeCredential(value: JsonRecord): JsonRecord {
  const encoded: JsonRecord = { ...value };
  if (isRecord(value.delegated_intent)) {
    encoded.delegated_intent = encodeHashField(
      value.delegated_intent,
      "intent_id",
      "validation.credential.delegated_intent.intent_id",
    );
  }
  if (isRecord(value.authority_chain)) {
    encoded.authority_chain = encodeHashField(
      value.authority_chain,
      "chain_hash",
      "validation.credential.authority_chain.chain_hash",
    );
  }
  if (Array.isArray(value.consent_refs)) {
    encoded.consent_refs = value.consent_refs.map((entry, index) =>
      isRecord(entry)
        ? encodeHashField(
            entry,
            "consent_id",
            `validation.credential.consent_refs[${index}].consent_id`,
          )
        : entry,
    );
  }
  if (Array.isArray(value.policy_refs)) {
    encoded.policy_refs = value.policy_refs.map((entry, index) =>
      isRecord(entry)
        ? encodeHashField(
            entry,
            "policy_id",
            `validation.credential.policy_refs[${index}].policy_id`,
          )
        : entry,
    );
  }
  if (typeof value.parent_avc_id === "string") {
    encoded.parent_avc_id = encodeHash(
      value.parent_avc_id,
      "validation.credential.parent_avc_id",
    );
  } else if (Array.isArray(value.parent_avc_id)) {
    encoded.parent_avc_id = requireByteArray(
      value.parent_avc_id,
      32,
      32,
      "validation.credential.parent_avc_id",
...
function encodeAction(value: JsonRecord): JsonRecord {
  ...
  if (isRecord(value.human_approval)) {
    ...
    if (approval.signature !== undefined) {
      approval.signature = encodeSignature(
        approval.signature,
        "validation.action.human_approval.signature",
      );
    }
...
function encodeHashField(value: JsonRecord, field: string, path: string): JsonRecord {
...
function encodeHash(value: string, path: string): WireHash256 {
...
function encodeSignature(value: unknown, path: string): WireSignature {
  if (typeof value === "string") {
    if (!ED25519_SIGNATURE_HEX.test(value)) {
      throw new LynkValidationError(
        `${path} must be a canonical 64-byte Ed25519 signature hex string`,
      );
    }
  ...
  if (Object.prototype.hasOwnProperty.call(value, "PostQuantum")) {
    return {
      PostQuantum: requireByteArray(
        value.PostQuantum,
        1,
        MAX_ML_DSA_65_SIGNATURE_BYTES,
        `${path}.PostQuantum`,
        false,
      ),
    };
  ...
  if (Object.prototype.hasOwnProperty.call(value, "Hybrid")) {
    ...
    return {
      Hybrid: {
        classical: requireByteArray(
          hybrid.classical,
          64,
          64,
          `${path}.Hybrid.classical`,
          true,
        ),
        pq: requireByteArray(
          hybrid.pq,
...
function decodeFixedHex(
  ...
  const bytes = Array.from({ length: expectedBytes }, (_unused, index) =>
...
function requireByteArray(
  ...
  if (
    ...
    throw new LynkValidationError(
      `${path} must contain ${minimumLength === maximumLength ? minimumLength : `${minimumLength}-${maximumLength}`} bytes`,
    );
  }
