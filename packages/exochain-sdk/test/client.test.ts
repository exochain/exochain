...
test('identity.resolve keeps a large malicious DID out of fetch and its error', async () => {
  ...
  const maliciousDid = `did:exo:${'a'.repeat(65_536)}/${marker}` as Did;
  const transport = jsonFetch({});
  const client = new ExochainClient({
    baseUrl: 'https://gateway.example',
    fetch: transport.fetch,
  ...
  strictEqual(
    ...
    false,
    'DID validation error must not echo input',
  );
  strictEqual(
    error.message,
    'DID method-specific identifier contains invalid characters',
  );
