...
3. Every release checkout is the validated workflow-dispatch commit and must initially be clean, including untracked files. Before build or package lifecycle execution, immutable source guards and transport helpers are captured with system Git and replacement objects disabled. Source inspection is anchored to `GITHUB_WORKSPACE`, rejects hidden index flags and tracked/staged drift, and neutralizes inherited shell/Git/loader controls. Once lifecycle execution starts, that job must not reload Git objects or guards or receive the repository token. Captured transport carries bounded outputs to fresh validator and publisher jobs, where exact source, artifact inventories and independently carried digests are checked. Live publishers rebind trusted identity inputs at step scope and verify source and tag before registry mutations, including each crate retry. A non-dry-run release requires an existing annotated, cryptographically verified signed `v<version>` tag whose peeled commit equals the dispatch commit. The isolated public keyring must contain exactly the configured primary key and its subkeys, and machine-readable signature evidence must bind the signer to that primary. The signed-tag gate records the immutable tag-object ID and peeled commit; live publication checks query both refs at the validated GitHub repository endpoint from outside the checkout with an empty inherited environment and disabled global/system Git configuration. The final GitHub Release creation step repeats the source and remote-tag comparisons. See `tools/test_release_workflow_ref_binding.sh` for the enforced lifecycle boundary.
