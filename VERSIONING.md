...
3. Every release checkout is the validated workflow-dispatch commit and must initially be clean, including untracked files. Immediately before each artifact or publication side effect, the workflow rechecks immutable `HEAD`; once expected build outputs exist, it still rejects staged or tracked-source drift. A non-dry-run release additionally requires an existing annotated, cryptographically verified signed `v<version>` tag whose peeled commit equals the workflow-dispatch commit and checked-out `HEAD`. The signed-tag gate records the immutable tag-object ID and peeled commit; every live side-effect boundary re-fetches the named remote tag and compares both values, including each retrying crate publication and the final GitHub Release creation step.
