...
"""Create or extract the strict raw WASM release-output transport."""
...
FILE_SIZE_LIMITS = {
    ".gitignore": 16 * 1024,
    "exochain_wasm.d.ts": 4 * 1024 * 1024,
    "exochain_wasm.js": 16 * 1024 * 1024,
    "exochain_wasm_bg.wasm": 64 * 1024 * 1024,
    "exochain_wasm_bg.wasm.d.ts": 4 * 1024 * 1024,
    "package.json": 256 * 1024,
}
MAX_ARCHIVE_SIZE = 96 * 1024 * 1024
MAX_INVENTORY_SIZE = 16 * 1024
INVENTORY_MAGIC = b"EXOCHAIN-WASM-TRANSPORT-V1"
...
def fail(message: str) -> NoReturn:
    raise SystemExit(f"WASM release transport failed: {message}")
...
def read_input_files(input_path: Path) -> tuple[Path, dict[str, bytes]]:
    ...
    try:
        ...
        if len(names) != len(EXPECTED_FILES) or set(names) != set(EXPECTED_FILES):
            fail("input directory must contain exactly the six expected files")
        ...
        for name in EXPECTED_FILES:
            ...
            try:
                ...
                payload = read_descriptor(descriptor, limit, f"input file {name}")
                after = os.fstat(descriptor)
                stable_fields = (
                    before.st_dev,
                    before.st_ino,
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Transport the exact raw EXOCHAIN WASM release output",
    )
